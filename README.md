...
Coderus 采用“人工分派、AI 执行”的协作模式：用户可通过控制面板或飞书机器人分派 Issue，由 AI Agent 自动完成问题分析、代码修复、测试验证和多角色代码检视，并从独立 Fork 向上游提交 Pull Request。用户还可以通过飞书查询任务进度、发起 PR 代码检视并接收结果通知，最终由维护者审核和决定是否合并。
